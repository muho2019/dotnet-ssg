...
- [x] `Models` 디렉토리 생성 및 데이터 모델 정의 (`SiteConfig.cs`, `Post.cs`, `Page.cs` 등)
...
- [x] 다수의 콘텐츠 파일을 병렬로 처리하여 빌드 성능 최적화
- [x] Front Matter의 메타데이터를 사용하여 `<title>`, `<meta>` 등 SEO 관련 태그 생성 기능 구현
- [x] 인덱스 페이지(포스트 목록) 생성 기능 구현
- [x] 태그별 아카이브 페이지 생성 기능 구현

### Phase 6: 마무리 및 배포

- [x] 빌드 프로세스를 실행하는 최종 `Program.cs` 로직 통합
- [x] 기본적인 오류 처리 및 로깅 추가
- [x] (선택) GitHub Actions를 통한 자동 빌드 및 배포 워크플로우 설정
